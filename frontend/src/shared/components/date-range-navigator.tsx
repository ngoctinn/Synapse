    ...
    if (value instanceof Date) {
      ...
      onChange({
        ...
        to: value?.to
          ? addDays(
              now,
              (value.to.getTime() - (value.from?.getTime() || 0)) /
                (1000 * 3600 * 24)
            )
          : undefined,
      });
    ...
    <div className={cn("flex flex-wrap items-center gap-2", className)}>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        <Button
          variant="outline"
          size="icon"
          className="border-border/40 text-muted-foreground hover:border-border hover:text-foreground size-9 transition-colors sm:size-10"
          onClick={handlePrev}
          aria-label={
            mode === "week"
              ? "Tuần trước"
        ...
        <Button
          ...
          disabled={
            disableFuture &&
            (value instanceof Date ? isSameDay(value, today) : false)
          }
          aria-label={
            mode === "week"
              ? "Tuần sau"
      ...
      {/* Divider */}
      <div className="bg-border/40 mx-1 hidden h-6 w-px sm:block" />
