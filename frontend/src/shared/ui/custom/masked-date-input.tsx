  ...
  ({ value, onChange, className, onInvalidInput, minDate, maxDate, icon: Icon, iconProps, error, ...props }, ref) => {
    ...
    React.useEffect(() => {
      if (value && !isNaN(value.getTime())) {
        ...
        if (inputValue === "" || (parts.length === 3 && !isNaN(Date.parse(`${parts[2]}-${parts[1]}-${parts[0]}`)))) {
           setInputValue("")
        }
      }
    }, [value])
      ...
      if (e.key === "Backspace") {
        ...
        if (selectionStart === 3 || selectionStart === 6) {
          ...
          requestAnimationFrame(() => {
            if (inputRef.current) {
              inputRef.current.setSelectionRange(selectionStart - 2, selectionStart - 2)
            }
          })
        }
      }
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (currentDigits.length > 0) {
        ...
        if (currentDigits.length > 2 || (currentDigits.length === 2 && val.includes('/'))) {
          formattedVal += "/"
        }
        ...
        if (currentDigits.length > 2) {
          ...
          if (currentDigits.length > 4 || (currentDigits.length === 4 && val.split('/').length > 2)) {
             formattedVal += "/"
          }
        }
