...
export function generateMockAppointments(
  ...
  const createApt = (
    ...
    return {
      ...
      status,
      isRecurring: false,
      createdAt: subDays(start, 3),
      updatedAt: subDays(start, 1),
      createdBy: "admin",
    };
  };
...
export function getMockMetrics(date: Date = new Date()): AppointmentMetrics {
  ...
  const occupiedSlots = todayApts.reduce(
    (acc, a) => acc + Math.ceil(a.duration / 15),
    0
  );
  const revenue = todayApts
    ...
    .reduce(
      (acc, a) =>
        acc + (MOCK_SERVICES.find((s) => s.id === a.serviceId)?.price || 0),
      0
    );
  ...
  return {
    ...
    noShowRate:
      todayApts.length > 0
        ? Math.round(
            (todayApts.filter((a) => a.status === "NO_SHOW").length /
              todayApts.length) *
              100
          )
        : 0,
