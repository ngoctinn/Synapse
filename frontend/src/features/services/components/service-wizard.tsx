...
interface ServiceWizardProps {
  ...
  mode: "create" | "edit";
  /** Dữ liệu ban đầu (chỉ dùng cho mode='edit') */
  initialData?: Service;
  /** Dialog open state */
  open: boolean;
  /** Callback khi thay đổi open state */
      ...
      try {
        ...
        if (result.success) {
          toast.success(
            ...
            { description: `Dịch vụ "${data.name}" đã được ${isEditMode ? "cập nhật" : "thêm vào hệ thống"}.` }
          );
          onOpenChange(false);
        } else {
        ...
        return (
          <React.Fragment key={s}>
            <button
              ...
              aria-label={STEP_CONFIG[s].title}
            >
              {isPast ? (
        ...
        {isEditMode && (
          <div className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Ẩn/Hiện dịch vụ trên app khách hàng</p>
            </div>
            <FormField
