...
export function ServiceFilter({ availableCategories }: ServiceFilterProps) {
  const { searchParams, updateParam, clearFilters } = useFilterParams({
    filterKeys: ["duration", "category_id"],
  });
  ...
  return (
    <HStack gap={2} className="items-center">
      ...
      <Select value={categoryId || "all"} onValueChange={handleCategoryChange}>
        <SelectTrigger className="bg-background w-56 text-sm">
      ...
      <Select value={duration || "all"} onValueChange={handleDurationChange}>
        <SelectTrigger className="bg-background w-48 text-sm">
        ...
        <SelectContent>
          ...
          <SelectItem value="0-30" className="text-xs">
            Dưới 30 phút
          </SelectItem>
          <SelectItem value="30-60" className="text-xs">
            30 - 60 phút
          </SelectItem>
          <SelectItem value="60-90" className="text-xs">
            60 - 90 phút
          </SelectItem>
          <SelectItem value="90-inf" className="text-xs">
            Trên 90 phút
          </SelectItem>
        </SelectContent>
      </Select>
