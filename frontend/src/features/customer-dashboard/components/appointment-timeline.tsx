...
// Interactive helper: Round time to nearest step
...
// Assigns columns to overlapping events
...
const calculateLayout = (appointments: Appointment[], startHour: number, endHour: number): LayoutItem[] => {
  ...
  // 2. Map to basic positions (top/height)
  ...
  // 3. Resolve overlaps (Simple greedy column packing)
  const columns: LayoutItem[][] = []

  items.forEach(item => {
    let placed = false
    // Find first column where this item fits without overlapping
    for (const column of columns) {
      const isOverlapping = column.some(existing =>
        (item.startMinutes < existing.endMinutes) && (item.endMinutes > existing.startMinutes)
      )
  ...
  // 4. Transform back to flat list with width/left
  const result: LayoutItem[] = []
  const totalColumns = columns.length

  if (totalColumns === 0) return []

  // Note: This is a simplified "Swimlane" view.
  // Ideally we want expand-to-fit, but fixed columns are safer for now to avoid complexity.
  // Visual improvement: Cluster dependent groups.
  // For now: Just simple columns.

  // Re-clustering logic for visual expansion:
  // Identify groups of colliding columns. That's too complex for this snippet.
  // Fallback: If totalColumns > 0, width = 100/totalColumns.

  // Re-do with clustering logic for better UX (Events expand if no neighbor)
  // Simplified Clustering:
  // We need to know for each item, what is the MAX overlapping degree at its time interval.

  // Let's stick to simple columns for v1 to ensure stability and correctness first.
  columns.forEach((col, colIndex) => {
    col.forEach(item => {
        // Find collisions for THIS generic cluster?
        // No, simple static columns for stability as requested in design.
        item.left = (colIndex / totalColumns) * 100
        item.width = (1 / totalColumns) * 100
        result.push(item)
    })
  })
...
export function AppointmentTimeline({
  ...
  const layoutItems = React.useMemo(() => {
    return calculateLayout(appointments, finalConfig.startHour, finalConfig.endHour)
  ...
  const handleGridClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    let clickTime = setMinutes(setHours(new Date(), finalConfig.startHour), minutes)
    clickTime = roundToNearestStep(clickTime, finalConfig.step)

    // Safety check: keep within bounds
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      let hoverDate = setMinutes(setHours(new Date(), finalConfig.startHour), minutes)
      hoverDate = roundToNearestStep(hoverDate, finalConfig.step)
      setHoverTime(format(hoverDate, "HH:mm"))
  }

  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault()
        // Default to current time if focused?
        // Complex to key-nav specific slots without more DOM nodes.
        // For now, allow focusing the appointments.
    }
  }
      ...
      <div className="flex-1 overflow-y-auto relative custom-scrollbar bg-background">
        <div className="flex min-h-[800px] h-full relative">
             ...
             <div className="w-16 shrink-0 flex flex-col border-r bg-muted/5 select-none z-20 bg-background" aria-hidden="true">
                {hours.map((hour) => (
                    <div key={hour} className="flex-1 border-b border-border/50 relative">
                        <span className="absolute -top-3 right-3 text-xs text-muted-foreground font-medium bg-background px-1">
                            {hour}:00
                        </span>
                    </div>
                ))}
             ...
             <div
                ...
                aria-label={`Lịch trình từ ${finalConfig.startHour} giờ đến ${finalConfig.endHour} giờ, ${appointments.length} cuộc hẹn`}
             >
                {/* Background Grid Lines */}
                {hours.map((hour) => {
                    const top = ((hour - finalConfig.startHour) * 60 / ((finalConfig.endHour - finalConfig.startHour) * 60)) * 100
                    return (
                        <div
                            key={hour}
                ...
                {hoverTime && (
                    <div
                        className="absolute w-full border-t-2 border-dashed border-primary/40 z-0 pointer-events-none flex items-center"
                        style={{ top: `${((parseInt(hoverTime.split(':')[0]) * 60 + parseInt(hoverTime.split(':')[1]) - finalConfig.startHour * 60) / ((finalConfig.endHour - finalConfig.startHour) * 60)) * 100}%` }}
                    >
                        <span className="bg-primary text-primary-foreground text-[10px] px-1.5 py-0.5 rounded-r ml-0 font-medium shadow-sm animate-fade-in">
                            {hoverTime}
                        </span>
                    </div>
                )}
                ...
                {currentTimeStyle && (
                    <div
                        className="absolute w-full border-t-2 border-red-500/80 z-20 pointer-events-none flex items-center transition-all duration-300"
                ...
                {layoutItems.map((app) => {
                    ...
                    return (
                        <div
                            key={app.id}
                            className={cn(
                                "absolute rounded-md border p-2 text-xs md:text-sm shadow-sm transition-all hover:shadow-md hover:z-30 cursor-pointer overflow-hidden group/item focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none",
                                status.bgClass,
                                status.borderClass
                            )}
                            style={{
                            ...
                            role="button"
                            tabIndex={0}
                            aria-label={`${app.serviceName} lúc ${format(new Date(app.startTime), "HH:mm")}, trạng thái ${status.label}`}
                            onKeyDown={(e) => {
                                if (e.key === 'Enter' || e.key === ' ') {
                                    e.preventDefault();
                                    onAppointmentClick?.(app);
                        ...
                        >
                            <div className="flex items-start justify-between gap-1 h-full relative">
                                ...
                                <div className="flex flex-col h-full pl-2.5 w-full">
                                    <span className={cn("font-semibold line-clamp-1 leading-tight text-[11px] md:text-sm", status.className)}>
                                        {app.serviceName}
                                    </span>
                                    {app.height > 5 && ( /* Only show extra info if block is tall enough */
                                        <div className="flex items-center gap-1.5 text-muted-foreground mt-1 text-[10px] md:text-xs">
                                            <Clock className="h-3 w-3" />
                                            <span className="truncate">
                                                {format(new Date(app.startTime), "HH:mm")} - {format(setMinutes(new Date(app.startTime), new Date(app.startTime).getMinutes() + app.durationMinutes), "HH:mm")}
                                            </span>
                                        </div>
                                    )}
