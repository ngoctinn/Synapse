...
export default function WorkspacePage() {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-8 pb-20 md:pb-8">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-black">Không gian làm việc</h1>
        <p className="text-sm text-muted-foreground">Chào buổi chiều! Bạn có 2 ca làm việc hôm nay.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
         <Card className="bg-primary/5 border-primary/10">
            <CardContent className="p-4 flex flex-col gap-2">
                 ...
                 <p className="text-[10px] text-muted-foreground uppercase font-bold">Xem lịch tuần</p>
            </CardContent>
         </Card>
         <Card className="bg-purple-500/5 border-purple-500/10">
            <CardContent className="p-4 flex flex-col gap-2">
                 ...
                 <p className="text-[10px] text-muted-foreground uppercase font-bold">3 ghi chú mới</p>
            </CardContent>
         </Card>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {todayTasks.map((task) => (
            <Card key={task.id} className="overflow-hidden border-none shadow-sm hover:shadow-md transition-shadow group">
               <CardContent className="p-0">
                  <div className="flex">
                     <div className={task.status === 'ongoing' ? "w-1.5 bg-primary" : "w-1.5 bg-muted"} />
                     <div className="flex-1 p-4">
                        <div className="flex justify-between items-start mb-2">
                           ...
                           <Badge variant={task.status === 'ongoing' ? 'default' : 'secondary'}>
                              {task.status === 'ongoing' ? 'Đang thực hiện' : 'Sắp tới'}
                           </Badge>
                        </div>
